...
from limited_vision_ai import LimitedVisionAI


class LimitedVisionInterface:
    """Terminal interface for the limited vision AI maze game."""

    def __init__(self, stdscr):
        """Initialize the limited vision interface."""
        self.stdscr = stdscr
        ...
        self.ai_player = LimitedVisionAI(self.maze)

        # Game state
        self.game_over = False
        self.won = False
        self.log_messages = ['Welcome to the Limited Vision Maze Adventure!']
        self.auto_mode = False
        self.auto_delay = 0.2  # seconds between moves in auto mode
        self.show_full_maze = False
        self.show_fog = True
    ...
    def _init_colors(self):
        ...
        curses.init_pair(19, curses.COLOR_WHITE, curses.COLOR_CYAN)  # Path
        ...
        curses.init_pair(25, curses.COLOR_BLACK, curses.COLOR_CYAN)  # Vision boundary
        curses.init_pair(
            26, curses.COLOR_BLACK, curses.COLOR_BLUE
        )  # Collected resource
        curses.init_pair(27, curses.COLOR_WHITE, curses.COLOR_BLACK)  # Unknown
    ...
    def draw_maze(self):
        ...
        if self.show_full_maze:
            ...
            viz_maze = self.ai_player.get_fog_of_war_visualization()
        else:
            viz_maze = self.maze.copy()
            # Mark the player's position
            player_y, player_x = self.ai_player.position
            ...
            for x in range(maze_width):
                if y + start_y < self.height and x * 2 + start_x + 1 < self.width:
                    ...
                    if cell_value == EnhancedCellType.WALL.value:
                        ...
                        symbol = 'AI'
                    elif cell_value == 98:  # Vision boundary
                        color = curses.color_pair(25)
                        symbol = '· '
                    elif cell_value == 97:  # Collected resource
                        color = curses.color_pair(26)
                        symbol = '✓ '
                    elif cell_value == 96:  # Path
                        color = curses.color_pair(19)
                        symbol = '→ '
                    elif cell_value == -1:  # Unknown
                        color = curses.color_pair(27)
                        symbol = '▒▒'
                    else:
    ...
    def draw_status(self):
        ...
        # Draw limited vision stats
        try:
            vision_line = f'Collected Resources: {status.get("collected_resources", 0)} | Known Resources: {status.get("known_resources", 0)} | Cells Seen: {status.get("cells_seen", 0)}'
            self.stdscr.addstr(start_y + 3, 2, vision_line, curses.color_pair(1))
        except curses.error:
            pass

        # Draw target info
        try:
            target_type = status.get('current_target_type')
            if target_type is not None:
                target_names = {
                    EnhancedCellType.RESOURCE_SMALL.value: 'Small Resource',
                    EnhancedCellType.RESOURCE_LARGE.value: 'Large Resource',
                    EnhancedCellType.END.value: 'Exit',
                    EnhancedCellType.PATH.value: 'Exploration',
                    EnhancedCellType.BOSS.value: 'Boss',
                    EnhancedCellType.SHOP.value: 'Shop',
                    EnhancedCellType.CHECKPOINT.value: 'Checkpoint',
                    EnhancedCellType.SECRET.value: 'Secret',
                }
                target_name = target_names.get(target_type, 'Unknown')
                target_line = f'Current Target: {target_name}'
                self.stdscr.addstr(start_y + 4, 2, target_line, curses.color_pair(1))
        except curses.error:
            pass
        ...
        if status['status_effects']:
            try:
                ...
                self.stdscr.addstr(start_y + 5, 2, effects_line, curses.color_pair(1))
            except curses.error:
                pass
    ...
    def draw_log(self):
        ...
        maze_height, maze_width = self.maze.shape
        start_y = max(0, (self.height - maze_height) // 2) + maze_height + 7
    ...
    def draw_menu(self):
        ...
        try:
            ...
            title = 'LIMITED VISION MAZE ADVENTURE'
            ...
            self.stdscr.addstr(8, 4, 'F - Toggle Fog of War', curses.color_pair(1))
            self.stdscr.addstr(9, 4, 'M - Toggle Full Maze View', curses.color_pair(1))
            self.stdscr.addstr(10, 4, 'S - Toggle Stats Display', curses.color_pair(1))
            self.stdscr.addstr(11, 4, 'H - Help', curses.color_pair(1))
            self.stdscr.addstr(12, 4, 'Q - Quit', curses.color_pair(1))
            ...
            fog_status = 'ON' if self.show_fog else 'OFF'
            self.stdscr.addstr(
                15,
                4,
                f'Fog of War: {fog_status}',
                curses.color_pair(1) | (curses.A_BOLD if self.show_fog else 0),
            )

            # Show full maze status
            maze_status = 'ON' if self.show_full_maze else 'OFF'
            self.stdscr.addstr(
                16,
                4,
                f'Full Maze: {maze_status}',
                curses.color_pair(1) | (curses.A_BOLD if self.show_full_maze else 0),
            )
        except curses.error:
            pass
    ...
    def draw_legend(self):
        ...
        try:
            ...
            legend_items = [
                ...
                (curses.color_pair(25), '· ', 'Vision Boundary'),
                (curses.color_pair(26), '✓ ', 'Collected Resource'),
                (curses.color_pair(19), '→ ', 'Planned Path'),
                (curses.color_pair(27), '▒▒', 'Unknown'),
            ]
        ...
        if self.game_over:
            if self.won:
                self.log_messages.append(
                    f'Victory! You have completed the maze with {status.get("collected_resources", 0)} resources!'
                )
            else:
                self.log_messages.append(
            ...
            try:
                ...
                if self.show_help:
                    ...
                    if key == ord('q') or key == ord('Q'):
                        ...
                        self.log_messages.append(
                            f'Fog of war {"shown" if self.show_fog else "hidden"}'
                        )
                    elif key == ord('m') or key == ord('M'):
                        self.show_full_maze = not self.show_full_maze
                        self.log_messages.append(
                            f'Full maze {"shown" if self.show_full_maze else "hidden"}'
                        )
                    elif key == ord('s') or key == ord('S'):
                        self.show_stats = not self.show_stats
                        self.log_messages.append(
                            f'Stats {"shown" if self.show_stats else "hidden"}'
                        )
                        ...
                        # Any other key advances the game
                        self.advance_game()
...
def main():
    """Main function to run the limited vision interface."""
    # Initialize curses
    curses.wrapper(lambda stdscr: LimitedVisionInterface(stdscr).run())
